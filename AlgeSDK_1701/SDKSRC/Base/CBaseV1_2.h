...
public:
  ...
  static bool PTInRect(float x1, float y1,CRect aRect)	{
    return ( (y1>=aRect.Bottom) && (y1<aRect.Top) && (x1>=aRect.Left) && (x1<aRect.Right) ) ;
  }
...
float fromScreenX(float x) { 
  return x - (screenRight - screenLeft)/ 2.0f;
}

float fromScreenY(float y) { 
  float nPixY = (screenTop-screenBottom);
...
public:
  ...
  void pushP(int command, void* param1, void* param2) {
...
public:
  ...
  void glDrawEx(bool wireframe_mode)
  {
	...
	glVertexPointer( 3, GL_FLOAT, 0, 0 );
      ...
      glDrawArrays(wireframe_mode?GL_LINE_STRIP:GL_TRIANGLES, 0, n_vertices);
#ifndef WIN32
      if (!noVBO) glBindBuffer(GL_ARRAY_BUFFER, 0);
#endif
    }
    else 
      glDrawElements(wireframe_mode?GL_LINE_STRIP:GL_TRIANGLES, n_indices, GL_UNSIGNED_SHORT, indices_array);
...
#define N 1
	...
	if (strcmp(line, "<source")==0) {
	  fscanf(file, "%s", line);
	  bool id_0 = strstr(line, "id=")>0;
	  bool id_pos = strstr(line, "Position")>0;
	  bool id_nrm = strstr(line, "Normal")>0;
	  bool id_uv = strstr(line, "UV")>0;
	  float* toPtr;
	...
	if (strstr(line, "-UV-array")>0) {
	  //	[self pickArray:file:models[modelId]->uv_array];
	}

	if (strcmp(line, "<triangles")==0) {
	  fscanf(file, "%s", line);
	  if(strstr(line, "count=")>0) models[modelId]->n_indices=atof(line+7)*3;
	  ...
	  do {
	    fscanf(file, "%s", line);
	    if(strstr(line, "offset")>0) models[modelId]->n_indices_accessors++;
